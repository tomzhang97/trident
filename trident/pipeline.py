...
class TridentPipeline:
    ...
    def process_query(
        ...
        # Enforce required facet coverage: abstain if any required facet lacks a certificate
        required_ids = {f.facet_id for f in facets if getattr(f, "required", False)}
        covered_ids = {
            c.get("facet_id")
            for c in (result.get('certificates') or [])
            if c.get("facet_id")
        }
        missing_required = required_ids - covered_ids
        if missing_required:
            metrics = result.get('metrics', {})
            metrics['abstention_reason'] = 'REQUIRED_FACETS_MISSING'
            metrics['missing_required_facets'] = list(missing_required)
    ...
    def _run_two_pass_safe_cover(
        ...
        if not bound_entity:
            if debug:
                print(f"  Failed to bind entity from hop-1 - abstaining (no binding)")
            hop1_metrics = hop1_result.get('metrics', {}) if isinstance(hop1_result, dict) else {}
            return {
                'selected_passages': [],
                'certificates': hop1_result.get('certificates', []) if isinstance(hop1_result, dict) else [],
                'abstained': True,
                'infeasible': True,
                'retrieval_tokens': hop1_result.get('retrieval_tokens', 0) if isinstance(hop1_result, dict) else 0,
                'evidence_tokens': hop1_result.get('evidence_tokens', 0) if isinstance(hop1_result, dict) else 0,
                'metrics': {
                    **hop1_metrics,
                    'abstention_reason': 'BINDING_FAILED',
                    'two_pass': True,
                },
            }
        ...
        # Auto-infer placeholder keys directly from hop-2 templates
        placeholder_re = re.compile(r"\[([A-Z_]+_RESULT)\]")
        for f in hop2_facets:
            for field in (f.template or {}).values():
                if isinstance(field, str):
                    for match in placeholder_re.findall(field):
                        if match not in bindings:
                            bindings[match] = bound_entity
